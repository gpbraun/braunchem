...
Os efeitos da penetração e da blindagem podem ser grandes. Um elétron $\mathrm{4s}$ costuma ter energia muito mais baixa do que um elétron $\mathrm{4p}$ ou $\mathrm{4d}$. Ele pode até ter energia inferior à de um elétron $\mathrm{3d}$ do mesmo átomo. A ordem precisa da energia dos orbitais depende do número de elétrons no átomo, como será explicado a seguir.

Por causa dos efeitos da penetração e da blindagem, a ordem das energias dos orbitais em uma dada camada em um átomo polieletrônico é $\mathrm{s} < \mathrm{p} < \mathrm{d} < \mathrm{f}$.
...
O átomo de hidrogênio tem, no estado fundamental, um elétron no orbital $\mathrm{1s}$. Essa estrutura é representada por uma seta no orbital $\mathrm{1s}$ de um *diagrama de caixas*, que mostra cada orbital como uma *caixa* que pode conter, no máximo, dois elétrons:
$$
    \ce{H}: \; 
    \underset{ \mathrm{1s^1} }{ \boxed{ \uparrow \hspace{5pt} } }
$$
Sua configuração eletrônica, isto é, uma lista dos orbitais ocupados, é representada por $\mathrm{1s^1}$ ("um $s$ um"). No estado fundamental do átomo de hélio ($Z = 2$), os dois elétrons estão em um orbital $\mathrm{1s}$, que é descrito como $\mathrm{1s^2}$ ("um $\ce{s}$ dois"):
$$
    \ce{He}: \; 
...
Quando um único elétron ocupa um orbital, escrevemos, por exemplo, $\mathrm{1s^1}$ e não $\mathrm{1s}$, de forma abreviada.

:::

O lítio ($Z = 3$) tem três elétrons. Dois elétrons ocupam o orbital $\mathrm{1s}$ e completam a camada $n = 1$. O terceiro elétron deve ocupar o próximo orbital de mais baixa energia disponível, o orbital $\mathrm{2s}$. Logo, o estado fundamental de um átomo de lítio é $\ce{Li}: \mathrm{1s^2 2s^1}$:
$$
    \ce{Li}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2s^1} }{ \boxed{ \uparrow \hspace{5pt} } } \;\; 
$$
Imagine que a estrutura eletrônica deste átomo é formada pelo caroço, parte central que contém os elétrons de orbitais totalmente preenchidos, circundado pelos elétrons de valência (os da camada mais externa). No lítio, o caroço é formado por uma camada fechada semelhante ao caroço $\mathrm{1s^2}$ do átomo de hélio, representado por $\ce{[He]}$. Este caroço é rodeado por uma camada externa que contém um elétron $\mathrm{2s}$, de energia mais alta. Assim, a configuração do lítio é $\ce{[He]}\,\mathrm{2s^1}$. Em geral, somente os elétrons de valência podem se envolver em reações químicas porque os elétrons do caroço estão nos orbitais internos de menor energia e, portanto, estão muito fortemente ligados. Isso significa que o lítio perde somente um elétron ao formar compostos, isto é, ele forma normalmente íons $\ce{Li^+}$, e não íons $\ce{Li^{2+}}$ ou $\ce{Li^{3+}}$.

O elemento com $Z = 4$ é o berílio, $\ce{Be}$, com quatro elétrons. Os primeiros três elétrons tomam a configuração $\mathrm{1s^2 2s^1}$, como o lítio. O quarto elétron emparelha-se com o elétron $\mathrm{2s}$, para dar a configuração $\mathrm{1s^2 2s^2}$, ou, mais simplesmente, $\ce{[He]}\,\mathrm{2s^2}$:
$$
    \ce{Be}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
$$
Um átomo de $\ce{Be}$ tem, então, um caroço semelhante ao do hélio, rodeado por uma camada de valência com dois elétrons emparelhados. Como o lítio --- e pela mesma razão ---, um átomo de $\ce{Be}$ só pode perder seus elétrons de valência em reações químicas. Assim, ele perde ambos os elétrons $\mathrm{2s}$ para
formar o íon $\ce{Be^{2+}}$.

O boro ($Z = 5$) tem cinco elétrons. Dois ocupam o orbital $1s$ e completam a camada $n = 1$. Dois ocupam o orbital $2s$. O quinto elétron ocupa um orbital da próxima subcamada disponível, que é um orbital $2p$. Este arranjo de elétrons corresponde à configuração $\mathrm{1s^2 2s^2 2p^1}$ ou $\ce{[He]}\,\mathrm{2s^2 2p^1}$:
$$
    \ce{Be}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2p^1} }{ \boxed{ \uparrow \hspace{5pt} }\hspace{-0.4pt}\boxed{ \phantom{\uparrow \downarrow} }\hspace{-0.4pt}\boxed{ \phantom{\uparrow \downarrow} } }
$$
que mostra que o boro tem três elétrons de valência em torno de um caroço semelhante ao do hélio.

Temos de tomar outra decisão no caso do carbono ($Z = 6$): o sexto elétron pode ficar junto ao anterior no orbital $\mathrm{2p}$ ou deve ocupar um orbital $\mathrm{2p}$ diferente? (Lembre-se de que existem três orbitais $p$ na subcamada, todos com a mesma energia.) Para responder essa questão, é preciso reconhecer que os elétrons estão mais longe um do outro e se repelem menos quando ocupam orbitais $\ce{p}$ diferentes do que quando ocupam o mesmo orbital. Portanto, o sexto elétron ocupa um dos orbitais $\mathrm{2p}$ vazios e o estado fundamental do carbono é $\mathrm{1s^2 2s^2 2p_x^1 2p_y^1}$
$$
    \ce{C}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
...
O nitrogênio tem $Z = 7$ e mais um elétron do que o carbono, o que dá $\ce{[He]}\,\mathrm{2s^2 2p^3}$. Cada elétron $p$ ocupa um orbital diferente e os três têm spins paralelos: 
$$
    \ce{N}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
...
$$
O oxigênio tem $Z = 8$ e mais um elétron do que o nitrogênio, logo, sua configuração é $\ce{[He]}\,\mathrm{2s^2 2p^4}$ e dois dos elétrons $\mathrm{2p}$ estão emparelhados:
$$
    \ce{O}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
...
$$
O flúor, por sua vez, com $Z = 9$, tem mais um elétron do que o oxigênio, e a configuração $\ce{[He]}\,\mathrm{2s^2 2p^5}$, com um elétron desemparelhado. 
$$
    \ce{F}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
    \underset{ \mathrm{2p^5} }{ \boxed{ \uparrow \downarrow }\hspace{-0.4pt}\boxed{ \uparrow \downarrow }\hspace{-0.4pt}\boxed{ \uparrow \hspace{5pt} } }
$$
O neônio, com $Z = 10$, tem mais um elétron do que o flúor. Esse elétron completa a subcamada $\mathrm{2p}$, levando a $\ce{[He]}\,\mathrm{2s^2 2p^6}$: 
$$
    \ce{Ne}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\; 
...
$$
De acordo com a Figura 1, o próximo elétron ocupa o orbital $\mathrm{3s}$, o orbital de menor energia na próxima camada. A configuração do sódio é, então, $\ce{[He]}\,\mathrm{2s^2 2p^6 3s^1}$, ou, de forma mais resumida, $\ce{[Ne]} \mathrm{3s^1}$, em que $\ce{[Ne]}$ é um caroço semelhante ao do neônio.
...
#### (a) Adicione $Z_{\ce{Mg}} = 12$ elétrons, um após o outro, aos orbitais na ordem mostrada na Figura 1, mas não coloque mais de dois elétrons em um mesmo orbital.

$$
    \ce{Mg}: \ce{[Ne]}\,\mathrm{ 3s^2 }
$$

#### (b) Adicione $Z_{\ce{Al}} = 13$ elétrons, um após o outro, aos orbitais na ordem mostrada na Figura 1, mas não coloque mais de dois elétrons em um mesmo orbital.
...
Os orbitais $s$ e $p$ da camada $n = 3$ se completam no argônio, $\ce{[Ne]}\,\mathrm{3s^2 3p^6}$, um gás incolor, sem cheiro, não reativo, que lembra o neônio. A energia do orbital $\mathrm{4s}$ é ligeiramente menor do que a dos orbitais $\mathrm{3d}$. Isso faz com que o próximo orbital a ser preenchido depois do $\pu{3p}$ seja o $\mathrm{4s}$. Então, as duas próximas configurações eletrônicas são $\ce{[Ar]}\,\mathrm{4s^1}$ para o potássio e $\ce{[Ar]}\,\mathrm{4s^2}$ para o cálcio, em que $\ce{[Ar]}$ representa um caroço semelhante ao do argônio, $\mathrm{1s^2 2s^2 2p^6 3s^2 3p^6}$. Neste ponto, entretanto, os orbitais $\mathrm{3d}$ começam a ser ocupados.

Em função da ordem do aumento da energia (Figura 1), os próximos 10 elétrons (do escândio, com $Z = 21$, até o zinco, com $Z = 30$) ocupam os orbitais $\mathrm{3d}$. A configuração eletrônica do estado fundamental do escândio, por exemplo, é $\ce{[Ar]}\,\mathrm{3d^1 4s^2}$, e a de seu vizinho, o titânio, é $\ce{[Ar]}\,\mathrm{3d^2 4s^2}$. Observe que, a partir do escândio, os elétrons $\mathrm{4s}$ são escritos depois dos elétrons $\mathrm{3d}$: assim que contiverem elétrons, os orbitais $\mathrm{3d}$ terão menor energia do que os orbitais $\mathrm{4s}$ (a mesma relação é verdadeira para os orbitais $n\mathrm{d}$ e $(n+1)\mathrm{s}$). Essas diferenças em energia são resultado das complexidades causadas pela repulsão entre elétrons. Os elétrons são adicionados sucessivamente aos orbitais $\ce{d}$ à medida que $Z$ aumenta. Entretanto, existem duas exceções: a configuração eletrônica experimental do cromo é $\ce{[Ar]}\,\mathrm{3d^5 4s^1}$, não $\ce{[Ar]}\,\mathrm{3d^4 4s^2}$, e a do cobre é  $\ce{[Ar]}\,\mathrm{3d^{10} 4s^1}$, não  $\ce{[Ar]}\,\mathrm{3d^9 4s^1}$. Essa discrepância aparente ocorre porque a configuração com a subcamada semipreenchida $\ce{d^5}$ e a configuração com a subcamada completa $\mathrm{d^{10}}$, segundo a mecânica quântica, têm energia mais baixa do que a indicada pela teoria simples. Como resultado, pode-se alcançar uma energia total mais baixa quando um elétron ocupa um orbital $\mathrm{3d}$, em vez do orbital $\mathrm{4s}$ esperado, se esse arranjo completa uma meia subcamada ou uma subcamada completa.

Os elétrons só ocupam os orbitais $\mathrm{4p}$ se os orbitais $\mathrm{3d}$ estiverem completos. A configuração do germânio, $\ce{[Ar]}\, \mathrm{3d^{10} 4s^2 4p^2}$, por exemplo, é obtida pela adição de dois elétrons aos orbitais $\mathrm{4p}$ depois de completar a subcamada $\mathrm{3d}$. O arsênio tem mais um elétron e a configuração é
$\ce{[Ar]}\,\mathrm{3d^{10} 4s^2 4p^3}$.

O próximo da fila para ocupação é o orbital $\mathrm{5s}$, seguido pelos orbitais $\mathrm{4d}$. Nesse caso, a energia do orbital $\mathrm{4d}$ cai abaixo da energia do orbital $\mathrm{5s}$ após a acomodação de dois elétrons no orbital $\mathrm{5s}$. Um efeito semelhante ocorre quando outro conjunto de orbitais internos, os orbitais $\mathrm{4f}$, começa a ser ocupado. O cério, por exemplo, tem a configuração $\ce{[Xe]}\,\mathrm{4f^1 5d^1 6s^2}$. Os elétrons continuam a ocupar os sete orbitais $\mathrm{4f}$, que se completam após a adição de 14 elétrons no itérbio, $\ce{[Xe]} \mathrm{4f^{14} 6s^2}$. Em seguida, os orbitais $\mathrm{5d}$ são ocupados. Os orbitais $\mathrm{6p}$ só são ocupados depois que os orbitais $\mathrm{6s}$, $\mathrm{4f}$ e $\mathrm{5d}$ estão completos no mercúrio. O tálio, por exemplo, tem configuração $\ce{[Xe]}\,\mathrm{4f^{14} 5d^{10} 6s^2 6p^1}$. Existe uma série de discrepâncias aparentes na ordem de preenchimento dos orbitais $\mathrm{4f}$. Essas discrepâncias aparentes ocorrem porque os orbitais $\mathrm{4f}$ e $\mathrm{5d}$ têm energias muito próximas. Na verdade, níveis de energia muito próximos são responsáveis pelo fato de
...
#### (a) Adicione $Z_{\ce{V}} = 23$ elétrons, um após o outro, aos orbitais na ordem mostrada na Figura 1, mas não coloque mais de dois elétrons em um mesmo orbital.

$$
    \ce{V}: \ce{[Ar]}\,\mathrm{ 3d^3 4s^2 }
$$

#### (b) Adicione $Z_{\ce{Pb}} = 82$ elétrons, um após o outro, aos orbitais na ordem mostrada na Figura 1, mas não coloque mais de dois elétrons em um mesmo orbital.

$$
    \ce{Pb}: \ce{[Xe]}\,\mathrm{ 4f^{14} 5d^{10} 6s^2 6p^2 }
$$

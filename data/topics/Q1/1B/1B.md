...
Os orbitais $s$ e $p$ da camada $n = 3$ se completam no argônio, $\ce{[Ne]}\,\mathrm{3s^2 3p^6}$, um gás incolor, sem cheiro, não reativo, que lembra o neônio. A energia do orbital $\mathrm{4s}$ é ligeiramente menor do que a dos orbitais $\mathrm{3d}$. Isso faz com que o próximo orbital a ser preenchido depois do $\pu{3p}$ seja o $\mathrm{4s}$. Então, as duas próximas configurações eletrônicas são $\ce{[Ar] 4s^1}$ para o potássio e $\ce{[Ar]}\,\mathrm{4s^2}$ para o cálcio, em que $\ce{[Ar]}$ representa um caroço semelhante ao do argônio, $\mathrm{1s^2 2s^2 2p^6 3s^2 3p^6}$. Neste ponto, entretanto, os orbitais $\mathrm{3d}$ começam a ser ocupados.

Em função da ordem do aumento da energia (Figura XX), os próximos 10 elétrons (do escândio, com $Z = 21$, até o zinco, com $Z = 30$) ocupam os orbitais $\mathrm{3d}$. A configuração eletrônica do estado fundamental do escândio, por exemplo, é $\ce{[Ar] 3d^1 4s^2}$, e a de seu vizinho, o titânio, é $\ce{[Ar] 3d^2 4s^2}$. Observe que, a partir do escândio, os elétrons $\mathrm{4s}$ são escritos depois dos elétrons $\mathrm{3d}$: assim que contiverem elétrons, os orbitais $\mathrm{3d}$ terão menor energia do que os orbitais $\mathrm{4s}$ (a mesma relação é verdadeira para os orbitais $n\mathrm{d}$ e $(n+1)\mathrm{s}$). Essas diferenças em energia são resultado das complexidades causadas pela repulsão entre elétrons. Os elétrons são adicionados sucessivamente aos orbitais $\ce{d}$ à medida que $Z$ aumenta. Entretanto, existem duas exceções: a configuração eletrônica experimental do cromo é $\ce{[Ar] 3d^5 4s^1}$, não $\ce{[Ar] 3d^4 4s^2}$, e a do cobre é  $\ce{[Ar]} \mathrm{3d^{10} 4s^1}$, não  $\ce{[Ar]}\,\mathrm{3d^9 4s^1}$. Essa discrepância aparente ocorre porque a configuração com a subcamada semipreenchida $\ce{d^5}$ e a configuração com a subcamada completa $d^{10}$, segundo a mecânica quântica, têm energia mais baixa do que a indicada pela teoria simples. Como resultado, pode-se alcançar uma energia total mais baixa quando um elétron ocupa um orbital $\mathrm{3d}$, em vez do orbital $\mathrm{4s}$ esperado, se esse arranjo completa uma meia subcamada ou uma subcamada completa.

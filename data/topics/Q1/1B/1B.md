...
$$
Sua configuração eletrônica, isto é, uma lista dos orbitais ocupados, é representada por $\mathrm{1s^1}$ ("um $s$ um"). No estado fundamental do átomo de hélio ($Z = 2$), os dois elétrons estão em um orbital $\mathrm{1s}$, que é descrito como $\mathrm{1s^2}$ ("um $\mathrm{s}$ dois"):
$$
    \ce{He}: \; 
...
$$
De acordo com a Figura XX, o próximo elétron ocupa o orbital $\mathrm{3s}$, o orbital de menor energia na próxima camada. A configuração do sódio é, então, $\ce{[He]}\, \mathrm{2s^2\, 2p^6\, 3s^1}$, ou, de forma mais resumida, $\ce{[Ne]} \mathrm{3s^1}$, em que $\ce{[Ne]}$ é um caroço semelhante ao do neônio.
...
Em função da ordem do aumento da energia (Figura XX), os próximos 10 elétrons (do escândio, com $Z = 21$, até o zinco, com $Z = 30$) ocupam os orbitais $\mathrm{3d}$. A configuração eletrônica do estado fundamental do escândio, por exemplo, é $\ce{[Ar]}\, \mathrm{3d^1\, 4s^2}$, e a de seu vizinho, o titânio, é $\ce{[Ar]}\, \mathrm{3d^2\, 4s^2}$. Observe que, a partir do escândio, os elétrons $\mathrm{4s}$ são escritos depois dos elétrons $\mathrm{3d}$: assim que contiverem elétrons, os orbitais $\mathrm{3d}$ terão menor energia do que os orbitais $\mathrm{4s}$ (a mesma relação é verdadeira para os orbitais $n\mathrm{d}$ e $(n+1)\mathrm{s}$). Essas diferenças em energia são resultado das complexidades causadas pela repulsão entre elétrons. Os elétrons são adicionados sucessivamente aos orbitais $\mathrm{d}$ à medida que $Z$ aumenta. Entretanto, existem duas exceções: a configuração eletrônica experimental do cromo é $\ce{[Ar]}\, \mathrm{3d^5\, 4s^1}$, não $\ce{[Ar]}\, \mathrm{3d^4\, 4s^2}$, e a do cobre é  $\ce{[Ar]}\, \mathrm{3d^{10}\, 4s^1}$, não  $\ce{[Ar]}\, \mathrm{3d^9\, 4s^1}$. Essa discrepância aparente ocorre porque a configuração com a subcamada semipreenchida $\mathrm{d^5}$ e a configuração com a subcamada completa $d^{10}$, segundo a mecânica quântica, têm energia mais baixa do que a indicada pela teoria simples. Como resultado, pode-se alcançar uma energia total mais baixa quando um elétron ocupa um orbital $\mathrm{3d}$, em vez do orbital $\mathrm{4s}$ esperado, se esse arranjo completa uma meia subcamada ou uma subcamada completa.

...
a. A primeira energia de ionização do boro é inferior à do berílio.
...
b. A afinidade eletrônica do berílio é muito menor que a do lítio.
c. A afinidade eletrônica do neônio é muito menor que a do flúor.

#### a. Tendência periódica diferente do esperado. Examine as configurações eletrônicas de todas as espécies.

Espera-se a liberação de mais energia quando um elétron é adicionado a um átomo de nitrogênio porque ele é menor do que um átomo de carbono e seu núcleo tem carga maior: a carga nuclear efetiva sobre os elétrons mais externos dos átomos neutros é $\pu{3,8}$ para $\ce{N}$ e $\pu{3,1}$ para $\ce{C}$. Entretanto, o oposto é observado, assim, você deve considerar também as cargas nucleares efetivas experimentadas pelos elétrons de valência dos ânions. Quando $\ce{C^-}$ se forma a partir de $\ce{C}$ o elétron adicional ocupa um orbital $\mathrm{2p}$ vazio:
$$
    \ce{C}: \; 
...
$$
O elétron adicional está bem separado dos demais elétrons p, assim, sofre uma carga nuclear efetiva próxima de $\pu{3,1}$. Quando $\ce{N^-}$ se forma a partir de $\ce{N}$, o elétron adicional deve ocupar um orbital $\mathrm{2p}$ que já está parcialmente cheio:
$$
    \ce{N}: \; 
    \underset{ \mathrm{1s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
    \underset{ \mathrm{2s^2} }{ \boxed{ \uparrow \downarrow } } \;\;
...
$$
A carga nuclear efetiva nesse íon é, portanto, muito menor do que $\pu{3,8}$ e, por isso, é necessário dar energia para a formação de $\ce{N^-}$ e a afinidade eletrônica do nitrogênio é menor do que a do carbono.

...
![Nomes das formas de moléculas simples e seus ângulos de ligação. Os pares isolados não foram incluídos porque não são levados em consideração na identificação das formas das moléculas.](1F-2D.svg)
...
- **Regra 1**: As regiões de altas concentrações de elétrons (ligações e pares isolados do átomo central) se repelem e, para reduzir essa repulsão, elas tendem a se afastar o máximo possível, mantendo a mesma distância do átomo central (Figura 2).

![Posições, de duas a sete, das regiões de alta concentração de elétrons (átomos e pares isolados) ao redor de um átomo central. Essas regiões são representadas por linhas retas que partem do átomo central. Use este diagrama para identificar o arranjo dos elétrons de uma molécula e depois use a Figura 1 para identificar sua forma a partir da localização dos átomos.](1F-11D.svg)
...
Quando existe mais de um átomo *central*, as ligações de cada átomo são tratadas de modo independente. Como exemplo, para predizer a forma de uma molécula de eteno, $\ce{CH2=CH2}$, cada átomo de carbono é considerado separadamente. Com base na estrutura de Lewis, observe que cada átomo de carbono tem três regiões de alta densidade eletrônica (duas ligações simples e uma ligação dupla):

![](1F-10D.svg)

O arranjo ao redor de cada átomo de carbono é, então, trigonal planar. Há dois átomos de $\ce{H}$ e um átomo de $\ce{C}$ ligados ao átomo de $\ce{C}$ central. Assim, você pode predizer que os ângulos $\ce{H-C-H}$ e $\ce{H-C-C}$ estarão próximos de $\pu{120}^\circ$. Essa estimativa é confirmada em laboratório (os valores medidos são  $\pu{117}^\circ$ e  $\pu{122}^\circ$, respectivamente). Observações experimentais também mostraram que os seis átomos estão no mesmo plano.

O modelo VSEPR trata ligações simples e múltiplas como equivalentes, não importa que estruturas de Lewis contribuam para a estrutura de ressonância que estamos considerando. Por exemplo, a descrição do íon nitrato, $\ce{NO3^-}$ baseada na ressonância diz que ele tem três ligações em torno do átomo central (Tópico 1D). Logo, você pode esperar que a molécula seja trigonal plana.
...
A **fórmula VSEPR** geral, $\ce{AX_nE_m}$, é útil para identificar as diferentes combinações de átomos e pares isolados ligados ao átomo central: A representa o átomo central, $\ce{X}$ um átomo ligado e $\ce{E}$ um par isolado. As moléculas de mesma fórmula VSEPR têm essencialmente o mesmo arranjo de elétrons e a mesma forma. Assim, ao reconhecer a fórmula, você pode predizer a forma (mas não necessariamente o valor numérico preciso dos ângulos de ligação que não são governados pela simetria). Por exemplo, a molécula $\ce{BF3}$, que contém três átomos de flúor ligados e nenhum par isolado em $\ce{B}$, é um exemplo de uma espécie $\ce{AX3}$, assim como o íon $\ce{NO3^{2-}}$. O íon sulfito, $\ce{SO3^{2-}}$, que tem um par de elétrons isolados no átomo de $\ce{S}$, é um exemplo de espécie $\ce{AX3E}$, assim como $\ce{NH3}$.

Se não existirem pares isolados no átomo central (uma molécula $\ce{AX_n}$), cada região de alta concentração de elétrons tem um átomo ligado, e a forma é a mesma do arranjo de elétrons. Se pares isolados de elétrons estão presentes, a forma da molécula é diferente da do arranjo de elétrons, porque somente as posições dos átomos ligados são levadas em consideração na determinação da forma. Por exemplo, as quatro regiões de alta concentração de elétrons no $\ce{SO3^{2-}}$ estarão suficientemente afastadas se adotarem o arranjo tetraédrico (Figura 2). Todavia, a forma do íon é determinada somente pela localização dos átomos. Como três dos vértices do tetraedro estão ocupados pelos átomos e um é ocupado pelo par isolado, o íon $\ce{SO3^{2-}}$ é uma pirâmide trigonal:

![](1F-12D.svg)

Uma regra importante é

- **Regra 3:** Todas as regiões de concentração eletrônica elevada e todos os pares de elétrons isolados e ligantes são incluídos na descrição do arranjo de elétrons. Todavia, somente as posições dos átomos são consideradas quando descrevemos a forma de uma molécula.

Um elétron desemparelhado no átomo central é tratado como uma região de alta concentração de elétrons e atua como um par isolado na determinação da forma da molécula. Assim, radicais como $\ce{NO2}$ têm um elétron não ligante. Logo, o $\ce{NO2}$ tem um arranjo trigonal planar de elétrons (incluindo o elétron desemparelhado de $\ce{N}$), mas sua forma, a configuração de seus átomos, é angular:

![](1F-13D.svg)

Até agora os pares de elétrons foram tratados como átomos quanto ao efeito na forma. Mas isso está correto? O arranjo dos elétrons do íon $\ce{SO3^{2-}}$ é tetraédrico. Por isso, é provável que os ângulos $\ce{O-S-O}$ sejam de $\pu{109,5}^\circ$. No entanto, observações experimentais mostraram que, embora o íon sulfito tenha estrutura de pirâmide trigonal, o ângulo de ligação é de apenas $\pu{106}^\circ$. Dados experimentais como esses mostram que o modelo VSEPR, como foi descrito, é incompleto e deve ser melhorado.
...
- **Regra 4:** A repulsão é exercida na ordem par isolado-par isolado > par isolado-átomo > átomo-átomo.
...
O modelo melhorado ajuda a explicar o ângulo de ligação do íon sulfito, $\ce{AX3E}$. Os pares de elétrons adotam um arranjo tetraédrico ao redor do átomo de $\ce{S}$. No entanto, o par isolado exerce uma forte repulsão sobre os elétrons ligantes, forçando-os a se aproximar. Como resultado desse ajuste de posições, o ângulo $\ce{O-S-O}$ se reduz de $\pu{109,5}^\circ$, do tetraedro regular, para $\pu{106}^\circ$ quando observado experimentalmente. Note que, embora o modelo VSEPR prediga a direção da distorção, ele não informa sua extensão. Você perceberá que, em qualquer espécie $\ce{AX3E}$, o ângulo $\ce{X-A-X}$ será menor do que $\pu{109,5}^\circ$, mas seu valor real não pode ser previsto: ele precisa ser medido experimentalmente ou calculado resolvendo a equação de Schrödinger
numericamente em um computador.

A Regra 4 permite predizer a posição em que o par isolado está. Por exemplo, o arranjo dos elétrons de uma molécula ou íon $\ce{AX4E}$, como o $\ce{SF4}$, é uma bipirâmide trigonal, mas existem duas posições possíveis para o par isolado:

- Um **par isolado axial** encontra-se no eixo da molécula, onde ele repele fortemente os pares de elétrons nas três ligações equatoriais, que estão separadas por ângulos de $\pu{90}^\circ$ em relação à posição axial.
- Um **par isolado equatorial** encontra-se no equador da molécula, no plano perpendicular ao eixo desta, onde ele repele fortemente apenas os pares de elétrons das duas ligações
axiais.
...
Uma molécula $\ce{AX3E2}$, como $\ce{ClF3}$, também tem um arranjo de bipirâmide trigonal de pares de elétrons, mas dois dos pares são pares isolados. Os dois pares estarão o mais afastado possível se ocuparem duas das três posições equatoriais, as quais estão em ângulo de $\pu{120}^\circ$, mas um pouco mais afastados um do outro. O resultado é uma molécula em forma de T:

![](1F-15D.svg)

Se os pares isolados estivessem nas posições axiais, eles formariam ângulos de $\pu{90}^\circ$ em relação à posição equatorial, resultando em maior repulsão.

Agora, vejamos uma molécula $\ce{AX4E2}$, que tem um arranjo octaédrico dos pares de elétrons, dois dos quais são pares isolados. Os dois pares estarão mais distantes se estiverem em posições opostas, levando a uma molécula em um arranjo quadrado planar:

![](1F-16D.svg)

Todas as moléculas de mesma fórmula VSEPR têm sempre a mesma forma, embora os ângulos de ligação possam ser um pouco diferentes. Por exemplo, $\ce{O3}$ é uma espécie $\ce{AX2E}$. Ele tem um arranjo de elétrons trigonal planar e uma forma molecular angular. O ângulo de ligação no $\ce{O3}$ é $\pu{116,8}^\circ$, valor este que é um pouco menor do que o valor previsto de $\pu{120}^\circ$ para uma molécula trigonal plana. O íon nitrito, $\ce{NO2^-}$, tem a mesma fórmula VSEPR geral (com ângulo de ligação de $\pu{116}^\circ$) e a mesma forma. O mesmo ocorre com o dióxido de enxofre, $\ce{SO2}$, cujo ângulo de ligação é $\pu{119,5}^\circ$. 

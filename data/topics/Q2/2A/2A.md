...
$$
    \Delta H^\circ 
        = \sum n \Delta H_{\mathrm{f}, \text{produtos}} - \sum n \Delta H_{\mathrm{f}, \text{reagentes}} 
    \tag{2}
$$
...
### Cálculo da entalpia de reação a partir das entalpias padrão de formação

As entalpias padrão de formação são comumente determinadas a partir de dados de
combustão, como mostra o exemplo a seguir.
...
Como os estados incial e final são os reagentes e produtos na temperatura de interesse, esse processo possui a mesma variação de entalpia do que a reação na temperatura de interesse. Esse cálculo é resumido pela lei de Kirchhoff:
$$
...
$$
    \Delta C_P = \sum C_{P, \text{produtos}} - \sum C_{P, \text{reagentes}}
$$
...
\begin{aligned}
    \Delta H_1^\circ
        ...
        &= \{ (\pu{30 J.K-1.mol-1}) + 3 (\pu{30 J.K-1.mol-1}) \} \times (\pu{298 K} - \pu{698 K}) \\
        &= \pu{-24 kJ.mol-1}
\end{aligned}
$$
...
\begin{aligned}
    \Delta H_2^\circ
        ...
        &= 2 (\pu{35 J.K-1.mol-1}) \times (\pu{698 K} - \pu{298 K}) \\
        &= \pu{14 kJ.mol-1}
\end{aligned}
$$

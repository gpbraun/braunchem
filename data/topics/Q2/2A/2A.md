...
Um calorímetro de volume constante foi calibrado com uma reação que libera $\pu{2 kJ}$ de calor em $\pu{0,1 L}$ de uma solução colocada no calorímetro, resultando em um aumento de temperatura de $\pu{4 \degree C}$. Em um experimento posterior, $\pu{50 mL}$ de uma solução $\pu{0,2 M}$ de $\ce{HCl}$ e $\pu{50 mL}$ de uma solução $\pu{0,2 M}$ de NaOH foram misturados no mesmo calorímetro e a temperatura subiu $\pu{1,2 \degree C}$. 
...
A Equação 2 é uma declaração **completa** de como variações da energia interna de um sistema fechado de composição constante podem ser obtidas: a única forma de mudar a energia interna de um sistema fechado é transferir energia para ele na forma de calor ou trabalho. Se o sistema está isolado, nem isso é possível, e a energia interna não pode mudar. Esta conclusão é denominada **primeira lei da termodinâmica**. Ela diz que:
...
Imagine um sistema em pressão constante, no qual a variação de energia interna é $\Delta U$ e a do volume é $\Delta V$. Da definição de entalpia na Eq. 3, temos que a variação de entalpia é
$$
...
$$
Por fim, como o sistema está aberto para a atmosfera ou em um recipiente que permite a mudança de volume, a pressão é igual à pressão externa, isto é, $P_\mathrm{ext} = P$, e os últimos dois termos se cancelam para dar $\boxed{ \Delta H = q }$.
...
$$
    \Delta H_\mathrm{vap} 
        = \Delta H(\text{vapor}) - \Delta H(\text{líquido})
\tag{6}
$$
...
$$
    \Delta H_\mathrm{fus} 
        = \Delta H(\text{líquido}) - \Delta H(\text{sólido})
\tag{7}
$$
A fusão, com uma única exceção conhecida (hélio), é endotérmica, logo, todas as entalpias de fusão (com a exceção mencionada) são positivas e não é preciso escrever o sinal positivo. A entalpia de fusão da água em $\pu{0 \degree C}$ é $\pu{6 kJ.mol-1}$. Vaporizar a mesma quantidade de água requer muito mais energia (acima
de $\pu{40 kJ}$). Na fusão, as moléculas podem se mover mais facilmente, mas permanecem próximas e, portanto, as interações entre elas são quase tão fortes como as do sólido.
...
$$
    \Delta H_\mathrm{fus} 
        = \Delta H(\text{vapor}) - \Delta H(\text{sólido})
$$

...
## O método dos tempos meias-vida consecutivos


:::derivation
### Como isso é feito?

Para determinar o tempo de meia-vida reagente $\ce{A}$ em uma reação de ordem $a$, escreva a lei de velocidade para o consumo de $\ce{A}$, na forma
...
$$
    \dfrac{ \mathrm{d}[\ce{A}] }{ [\ce{A}]^a } 
        = -k \mathrm{d} t
$$

Em seguida, integre ambos os lados entre os limites $t = 0$ (quando $[\ce{A}] = [\ce{A}]_0$) e o tempo de meia-vida, $t_{1/2}$ (quando $[\ce{A}] = \frac{1}{2}[\ce{A}]_0$):
...
$$
    \int_{[\ce{A}]_0}^{[\ce{A}]_t} \dfrac{ \mathrm{d}[\ce{A}] }{ [\ce{A}]^a }  
        = \dfrac{1}{a-1} \left( \dfrac{1}{[\ce{A}]_0^{a-1}} - \dfrac{1}{[\ce{A}]_t^{a-1}} \right)
$$

Portanto,

$$
    \dfrac{1}{[\ce{A}]_t^{a-1}} = \dfrac{1}{[\ce{A}]_0^{a-1}} + (a-1) kt
$$

Fazendo-se $t = t_{1/2}$ e $[\ce{A}] = \frac{1}{2} [\ce{A}]_0$ e resolvendo para $t_{1/2}$. A expressão resultante,
...
### Determinação da ordem da reação pelo método das meias-vidas

...
O mol é a unidade utilizada para medir a propriedade física formalmente chamada de **quantidade de substância**, $n$. Esse nome, porém, é pouco usado pelos químicos, que preferem referir-se a ela, coloquialmente, como **número de mols**.
...
#### Calcule da massa molar de $\ce{H2SO4}$.
...
$$
\begin{aligned}
   M &= \dfrac{ (\pu{0,48 g//L}) \times (\pu{62,4 L.Torr//K.mol}) \times (\pu{533 K}) }{ (\pu{103 Torr}) } \\

...
#### Calcule a concentração de $\ce{H2C2O4}$ em solução.
...
O nitrogênio em uma amostra de $\pu{10 g}$ de farinha de trigo foi integralmente convertido em amônia pela adição de uma base concentrada após digestão com $\ce{H2SO4}$. A amônia foi coletada em $\pu{30 mL}$ de $\ce{HCl}$ $\pu{0,4 mol.L-1}$. O excesso de $\ce{HCl}$ foi retrotitulado com $\pu{5 mL}$ de hidróxido de cálcio, $\ce{Ca(OH)2}$, $\pu{0,2 mol.L-1}$.

**Calcule** a fração mássica de nitrogênio na farinha.

#### Escreva as equações das reações ácido base.

$$
\begin{aligned}
    \ce{ NH3(g) + HCl(aq) &-> NH4Cl(aq) } \\
    \ce{ 2 HCl(aq) + Ca(OH)2(aq) &-> CaCl2(aq) + 2 H2O(l) }
\end{aligned}
$$

#### Calcule a quantidade adicionada de $\ce{HCl}$ e $\ce{Ca(OH)2}$.

De $n = cV$
$$
\begin{aligned}
    n_{\ce{HCl}}
        &= (\pu{0,4 mol//L}) \times (\pu{30 mL}) 
        = \pu{12 mmol} \\
    n_{\ce{Ca(OH)2}}
        &= (\pu{0,2 mol//L}) \times (\pu{5 mL}) 
        = \pu{1 mmol} \\
\end{aligned}
$$

#### Use as relações estequiométricas para converter a quantidade de $\ce{HCl}$ nas quantidades de $\ce{NH3}$ e $\ce{Ca(OH)2}$.

$$
    n_{\ce{HCl}} 
        = n_{\ce{NH3}} + 2 n_{\ce{Ca(OH)2}}
$$
logo,
$$
    n_{\ce{NH3}} 
        = \pu{12 mmol} - 2\times \pu{1 mmol}
        = \pu{10 mmol}
$$

#### Converta a quantidade de $\ce{NH3}$ na quantidade de $\ce{N}$.

Como cada molécula de $\ce{NH3}$ contém um átomo de $\ce{N}$
$$
    n_{\ce{N}} = \pu{10 mmol}
$$

#### Converta a quantidade de $\ce{N}$ em massa.

De $m = nM$
$$
    m_{\ce{N}} = (\pu{10 mmol}) \times (\pu{14 g//mol}) = \pu{140 mg}
$$

#### Calcule a fração mássica de $\ce{N}$ na amostra.

De $f_{\ce{N}} = m_{\ce{N}}/m$
$$
    f_{\ce{N}} = \dfrac{ \pu{140 mg} }{ \pu{10 g} } = \boxed{ \pu{1,4}\% }
$$
...
Ligantes quelatos **sequestram** cátions metálicos ligando-se a eles e removendo-os de solução. Esses ligantes podem ser usados para solucionar problemas ambientais e para determinar a concentração de cátions metálicos em solução.

> As reações de complexação normalmente envolvem a substituição dos ligantes em um complexo.

## A volumetria de complexação

As reações de formação de complexos apresentam diversas utilidades em química analítica, mas sua aplicação clássica está nas **titulações de complexação**. Nessas titulações um íon metálico reage com um ligante adequado para formar um complexo. O $\ce{EDTA}$ é o titulante complexométrico mais utilizado. Cada molécula de EDTA, $\ce{Y^{4-}}$, forma um complexo com exatamente um cátion do metal, por exemplo:
$$
    \ce{ Zn^{2+}(aq) + Y^{4-}(aq) -> ZnY^{2-}(aq) }
$$

O Negro de Eriocromo T é um indicador típico de íons metálicos que é utilizado na titulação de diversos cátions comuns por formar complexos vermelhos na presença desses íons.

:::example

### Cálculo da concentração por titulação de complexação

Uma alíquota de $\pu{50 mL}$ de uma solução contendo nitrato de níquel, $\ce{Ni(NO3)2}$, foi titulada com $\pu{10 mL}$ de uma solução $\pu{0,05 mol.L-1}$ de $\ce{EDTA}$.

**Calcule** a concentração de cátions níquel na solução.

#### Escreva a equação iônica simplificada da reação de complexação.

$$
    \ce{ Ni^{2+}(aq) + Y^{4-}(aq) -> NiY^{2-}(aq) }
$$

#### Calcule a quantidade adicionada de EDTA.

$$
    n_{\ce{EDTA}} 
        = (\pu{0,05 mol//L}) \times (\pu{10 mL})
        = \pu{0,5 mmol}
$$

#### Use a relação estequiométrica para converter a quantidade de EDTA na quantidade de $\ce{Ni^{2+}}$.

$$
    n_{\ce{Ni^{2+}}} = \dfrac{1}{1} \times \pu{0,5 mmol} = \pu{0,5 mmol}
$$

#### Calcule a concentração de $\ce{Ni^{2+}}$ em solução.

De $c = n/V$
$$
    c_{\ce{Ni^{2+}}} 
        = \dfrac{ \pu{0,5 mmol} }{ \pu{50 mL} } 
        = \boxed{ \pu{0,01 mol.L-1} }
$$

:::

> Em uma titulação de complexação, o analito normalmente é um cátion metálico e o titulante é uma base de Lewis.

...
def html2md(content):
    ...
    return convert_text(content, "commonmark_x", format="html+tex_math_dollars+raw_tex")


def md2html(content):
    """Convert markdown to html using pandoc and parse as soup."""
    return convert_text(content, "html", format="markdown-tex_math_dollars-raw_tex")
...
def md2tex(content):
    ...
    tex = convert_text(
        content,
        "tex",
        format=f"markdown_strict+tex_math_dollars+raw_tex{PANDOC_EXTENSIONS}",
    )

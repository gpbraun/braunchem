...
*Exemplo: Identificação dos agentes oxidantes e redutores*

# A representação das reações redox

## As semi-reações

Uma meia-reação é a parte de oxidação ou de redução de uma reação redox considerada separadamente. Uma meia-reação de oxidação mostra a remoção de elétrons de uma espécie que está sendo oxidada. Por exemplo, para mostrar apenas a oxidação do zinco na reação do metal com íons prata,
$$
...
$$
    \ce{ Zn(s) -> Zn^{2+}(aq) + 2e^- }
$$
É muito importante entender que uma meia-reação de oxidação é meramente uma maneira conceitual de representar uma oxidação: os elétrons nunca estão realmente livres. Na equação de uma meia-reação de oxidação, os elétrons perdidos sempre aparecem do lado direito da seta. Seu estado não é dado porque eles estão em trânsito e não têm um estado físico definido. As espécies reduzida e oxidada, juntas, formam um par redox. Nesse exemplo, o par redox é $\ce{Zn^{2+}}$ e $\ce{Zn}$ e é representado por $\ce{Zn^{2+}/Zn}$. Um par redox tem sempre a forma $\ce{Ox/Red}$,
...
$$
    \ce{ Ag^+(aq) + e^- -> Ag(s) }
$$
Essa meia-reação também é conceitual: os elétrons nunca estão realmente livres e seus estados não são especificados. Na equação de uma meia-reação de redução, os elétrons ganhos sempre aparecem à esquerda da seta. Nesse exemplo, o par redox é $\ce{Ag^+/Ag}$.
...
*Exemplo: Cálculo do volume de ácido necessário para dissolver uma liga metálica.*
...
*Exemplo: Problema da mistura de $\ce{ Fe^{2+}(aq)}$ e $\ce{ Fe^{3+}(aq)}$*
